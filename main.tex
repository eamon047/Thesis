...
\begin{eabstract}
This thesis studies predictive multiplicity in knowledge graph embedding based link prediction.
Aggregate ranking metrics such as Hits@10 summarize average performance, but they do not show whether individual query-level decisions are stable across similarly performing models.
Predictive multiplicity addresses this issue by measuring disagreement among comparable prediction outputs.
This thesis further asks whether such instability is uniformly distributed or systematically associated with relation-level graph structure.

Using FB15k-237 and repeated runs of RotatE and TransE, this thesis analyzes relation-level Hits@10, ambiguity, and discrepancy under several structural factors.
The main finding is that predictive multiplicity is strongly side-dependent under relation mapping type: structurally harder prediction directions tend to have lower Hits@10 and higher ambiguity and discrepancy.
Inverse-like support is studied as an exploratory secondary factor, while symmetry is treated as a plausible but weakly supported candidate factor.
The final abstract will be revised after the main experimental chapters are finalized.
\end{eabstract}
...
TODO: Write the acknowledgement after the thesis body is stable.
