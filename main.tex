...
\begin{eabstract}
Knowledge graph embedding (KGE) models predict missing links by ranking
candidate entities. For a given KGE method, repeated runs trained with the same
configuration but different random seeds can achieve similar overall accuracy
while disagreeing on whether the correct entity appears in the top ten for the
same query. This phenomenon is known as predictive multiplicity. This means
that the outcome for an individual query may depend on random training choices
even when overall performance appears stable. Previous work established and
measured predictive multiplicity in KGE-based link prediction, but its main
analysis left open whether some types of queries are more affected than others.

This thesis examines which types of link-prediction queries are more likely to
be unstable. Using FB15k-237, we analyze RotatE and TransE separately. For each
method, we compare repeated runs that use the same configuration and differ
only in random seed. We compare top-10 accuracy and disagreement across runs
for three relation patterns: whether a relation connects one or many entities
on each side (mapping type), whether a different relation connects the same
entities in reverse order (inverse), and whether the relation itself connects
entity pairs in both directions (symmetry). The results show that prediction
instability differs across relation patterns in both models, most clearly for
mapping type. This analysis may help future work focus on the types of queries
for which predictions are less stable.
\end{eabstract}
